...
PCA9685.prototype.moveServo = function (num, val) {
  ...
  console.log(Math.floor(Constants.MAX/2-(Constants.MAX/2)*(val/180)),Math.floor((Constants.MAX/2)+((Constants.MAX/2)*(val/180))));
  this.setPWM(num, Math.floor(Constants.MAX/2-(Constants.MAX/2)*(val/180)), Math.floor((Constants.MAX/2)+((Constants.MAX/2)*(val/180))));
};
